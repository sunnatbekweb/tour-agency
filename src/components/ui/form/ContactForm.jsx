...
export const ContactForm = ({ setSuccess }) => {
	...
	const handleSubmit = async e => {
		...
		try {
			await axios.post(
				`${process.env.NEXT_PUBLIC_BACKEND_URL}/blog/contact/`,
				formData
			)
			setFormData({
				full_name: '',
	...
	return (
		<>
			<form
				...
				className="px-8 py-10 bg-white rounded-2xl"
			>
				<h4 className="font-medium text-[28px] lg:text-[36px] mb-6 md:mb-10 lg:mb-15">
					{t('title')}
				</h4>
				<label
					htmlFor="full_name"
				...
				<button className="w-full sm:w-75 h-12 lg:h-14 grid place-content-center text-white bg-[#A5958B] hover:opacity-75 active:opacity-100 mx-auto rounded-lg md:rounded-4xl font-medium text-base lg:text-xl mt-8 lg:mt-15 duration-300">
					{t('send')}
				</button>
			</form>
		</>
