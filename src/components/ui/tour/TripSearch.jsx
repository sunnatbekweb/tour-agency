...
export function TripSearch() {
	...
	const getTripTheme = async () => {
		try {
			await axios
				.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tour/trip-theme/`)
			...
			>
				...
				>
					...
					<select
						...
						className={`w-full h-12 text-sm md:text-base lg:h-17.5 focus:outline-none cursor-pointer ${filterData.destination === '' ? 'text-[#BDBDBD]' : 'text-[#323232]'}`}
					>
						<option
							value=""
							disabled
				...
				>
					...
					<select
						...
						onChange={handleChange}
						className={`w-full h-12 text-sm md:text-base lg:h-17.5 focus:outline-none cursor-pointer ${filterData.trip_theme === '' ? 'text-[#BDBDBD]' : 'text-[#323232]'}`}
					>
						<option
							value=""
							disabled
