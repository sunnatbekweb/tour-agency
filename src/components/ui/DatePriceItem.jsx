            ...
            <div className="accordion__content-body-item flex flex-col gap-y-6 md:gap-y-8">
              <div className="flex flex-col gap-y-2 md:gap-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="md:text-2xl text-[#878787]">{props?.[`dates_${locale}`]}</h4>
                  <button
                  ...
                  >
                    {props?.ticket_type === 'sold_out' ? 'Sold out' : 'Avilable'}
                  </button>
                </div>
              </div>
              <div className="flex flex-col gap-y-1">
                {[...Array(1)].map((_, index) => (
                  <div key={index} className="flex items-center justify-between text-[#878787]">
                    <h5 className="text-sm md:text-lg">Tour double occupancy</h5>
